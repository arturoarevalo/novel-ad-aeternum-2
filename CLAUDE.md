...
- Fase actual: **G-A1 APROBADO (2026-08-17); Fase 2 fusionada en `main`. W2 NO ha arrancado: esperar la orden del autor** (tras reiniciar la sesión con los agentes en `claude-opus-5`). Paquete: 47 OT + `OT-25b` (R2) en `ordenes/`, `ordenes/RESERVA.md` (R1–R5 aprobadas), `informes/f2-mapa-intervenciones.md`, `informes/g-a1-gate.md`; decisiones en `informes/registro-gates-autor.md`. Baseline de anti-regresión de v0 con el jurado vigente (A6-1/2 `claude-opus-5` + A6-3 `claude-opus-4-8`): `informes/a6-v0-baseline-opus5.md` (si no existe aún, la crea A0 al arrancar con `critica-fria.sh`); la baseline anterior (fable) en `informes/d1-aislamiento.md` §4 queda como histórica. Siguiente paso cuando el autor lo ordene: rama `w2-reescrituras`, `inyectar-frontmatter.sh --set cap-NN.md estado=en_oleada` (8, 13, 17, 21, 30, 36) y W2 en el orden 8, 13, 30, 17, 21, 36 (A3a → A4 → A5 → A8 → lector frío ×6 + A6×3 frío → gate W2). Checkpoint exacto en `informes/estado-proceso.md`. Actualiza esta línea al cerrar cada fase/oleada.
